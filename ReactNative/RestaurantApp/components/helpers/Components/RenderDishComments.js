...
export default function RenderComments({ comments }) {
  ...
  const renderCommentItem = ({ item, key }) => (
    <View key={key} style={{ margin: 10 }}>
      ...
      <View style={{ margin: 10, flex: 1, flexDirection: 'row' }}>{renderStars(item.rating)}</View>
      <Text style={{ fontSize: 12 }}>{`-- ${item.author}, ${formatter.format(Date.parse(item.date))}`}</Text>
    </View>
  );
